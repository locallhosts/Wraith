...
type Server struct { Store store.Store; WebhookSecret string; RulesDir string; OutputDir string; Slack *notify.SlackNotifier; TrustedSigningKey ed25519.PublicKey; Log *slog.Logger; PipelineTrigger func(runID,rulePath string); RateLimit gin.HandlerFunc }
